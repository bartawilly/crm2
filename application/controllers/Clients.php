...
class Clients extends MY_Controller {
    ...
    function save() {
        ...
        if ($this->login_user->is_admin) {
            $data["currency_symbol"] = $this->input->post('currency_symbol') ? $this->input->post('currency_symbol') : "SAR";
            $data["currency"] = $this->input->post('currency') ? $this->input->post('currency') : "SAR";
    ...
    function add_new_file_modal_form() {
     
        $view_data['client_id']= $this->input->post('client_id');

        echo"<script>console.log(".$view_data['client_id'].");</script>";
    ...
    $config = array(
        ...
        'allowed_types' => "png|PNG|jpg|pdf|jpeg|docx|dwg|dxf",
        'encrypt_name'=>true
    );
    ...
    if($this->upload->do_upload())
    {
        ...
        $client_id = $this->input->post('client_id');
         ...
         $data_details = array(
            ...
            $save_id = $this->Clients_files_model->insert_file($data_details);
      ...
      $data['error'] = array('error' => $this->upload->display_errors());
        ...
        if ($save_id){
             echo json_encode(array("success" => true, "data" => $this->_file_row_data($save_id), 'id' => $save_id, 'message' => lang('record_saved')));
          }else {      
            echo json_encode(array("success" => false, 'message' => lang('error_occurred')));         
       }
}
    ...
    function files_list_data($client_id = 0) {
        ...
        $list_data = $this->Clients_files_model->get_details($client_id)->result();
        $result = array();
        foreach ($list_data as $data) {
     ...
     private function _file_row_data($id) {
        $data = $this->Clients_files_model->get_details_id($id)->row();
    ...
    private function _make_file_row($data) {
        ...
        return array(
            ...
            js_anchor("<i class='fa fa-times fa-fw'></i>",
                   array('title' => lang('delete_document'), 
                         "class" => "delete", "data-id" => $data->id,
                         "data-action-url" => get_uri("Clients/delete_file"),
                         "data-action" => "delete"))
            ."<a href='".$this->config->base_url()."index.php/Clients/download_file?id=".$data->id."' target='_blank' class='edit'  title = 'download file' > <i class='fa fa-download fa-fw'></i></a>",
    ...
    function delete_file() {
        ...
        echo $id;             
        $file = $this->Clients_files_model->get_details_id($id)->row();
        $filename = $file->path;
